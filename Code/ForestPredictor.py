...
for idx in range(1,2):
    temp = [curr,curr+99]
    temp_input, temp_output = combineData([1,18],"song_csv_test/input/input_data_","song_csv_test/output/output_data_")
    try:
...
clf.fit(training_input_data, training_output_data)

test_input_data = pd.DataFrame(test_input_data)
y_prime = clf.predict(test_input_data)

confusion_matrix_test = confusion_matrix(test_output_data,y_prime,labels=[0,1,6,7])
df_cm = pd.DataFrame(confusion_matrix_test, index = [i for i in [0,1,6,7]],
                  columns = [i for i in [0,1,6,7]])
plt.figure(figsize = (10,7))
sn.heatmap(df_cm, annot=True)
plt.savefig("Output/test_confusion_matrix_forest.pdf")

accuracy = clf.score(test_input_data,test_output_data)
print "Random Forest Gives accuracy score of:"+str(accuracy)